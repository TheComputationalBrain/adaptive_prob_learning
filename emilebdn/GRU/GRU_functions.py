"""
This module provides utilities for processing and analyzing sequences of experimental data,
training GRU and RNN models, and evaluating their performance. It includes functions for
data formatting, sequence flattening, model training, evaluation, and visualization of
subject sequences with model predictions. The module supports both subject-level and
group-level analysis and includes visualization tools for comparing model predictions
with actual subject behavior.
...
from emilebdn.GRU.GRU_models import SimpleGRU, SimpleRNN
...
def format_sequences(path, task, data=None):
    """
    Import sequences from experimental or simulated data and optionally attach subject labels for GRU training.
    """
    if data is None:
...
def flatten_sequences(sequences, tested_sequence=None):
    """
    Flatten a list of sequences into a single DataFrame, which is essentially the reverse action of import_sequences.
    This function prepares the data for fitting HMM models using the data objects that were used to fit GRU models.
...
def train_and_evaluate_gru(train_sequences, test_sequences, input_size=input_size, hidden_size=hidden_size,
                           ...
                           batch_size=batch_size, return_pred=False, vanilla_RNN=False):
    """
