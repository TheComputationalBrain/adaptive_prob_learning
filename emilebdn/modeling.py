...
all_updates = Parallel(n_jobs=n_jobs)(
    delayed(process_subject)(subject, i) for subject, i in subjects_nb.items()
)
...
results = Parallel(n_jobs=n_jobs)(
    delayed(subject_gru_prediction)(subject, i) for subject, i in subjects_nb.items()
)
...
results = Parallel(n_jobs=n_jobs)(
    delayed(hmm_subject_prediction)(subject, i) for subject, i in subjects_nb.items()
)
...
#%%
### 7 - Group-wise HMM Predictions - IN PROGRESS
### with (for train sequences) all sequences 
### except the sequences with the same outcome as the test sequences, 
### except the sequences with the same subject than the test sequences' one, 
### if model == 'group_HMM':
...
data = pd.read_csv(data_path)
model = 'group_HMM' # 'group_HMM' or 'big_group_HMM'
data[model] = np.full(n_sb*n_seq*n_tri, np.nan)
...
results = Parallel(n_jobs=n_jobs)(
    delayed(hmm_group_prediction)(subject, i) for subject, i in subjects_nb.items()
)
...
results = Parallel(n_jobs=n_jobs)(
    delayed(process_subject_group_augmented_HMM)(subject, i) for subject, i in subjects_nb.items()
)
