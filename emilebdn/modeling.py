"""
Script for Predicting Subject Behavior in a Probability Learning Task

This script utilizes various computational models to predict subject behavior
in a probability learning task. The task involves analyzing sequences of trials
to estimate the probability of outcomes based on different models. The script
employs several approaches, including mean estimation, vanilla Recurrent Neural Networks
(RNN), Gated Recurrent Units (GRU), and Hidden Markov Models (HMM).
...
#%%
### 2 - Subject_wise vanilla RNN or GRU Predictions

vanilla_RNN = True # If True, use vanilla RNN, else use GRU

for hidden_size in [32, 512, 1024, 2048]:
    data = pd.read_csv(data_path)

    if vanilla_RNN:
        model = f'subject_RNN_{hidden_size}'
    else:
    ...
    def subject_gru_prediction(subject, i):
        ...
        for m in range(nb_test_seqs):
            ...
            GRU_subject = train_and_evaluate_gru(train_sequences, test_sequences, hidden_size=hidden_size, return_pred=True, vanilla_RNN=vanilla_RNN)['sequence_pairs']
...
#%%
### 3 - Group-wise vanilla RNN or GRU Predictions
...
for hidden_size in [32, 512, 1024, 2048]:
    ...
    if vanilla_RNN:
        model = f'group_RNN_{hidden_size}'
    else:
    ...
    def group_gru_prediction(subject, i):
        ...
        for m in range(nb_test_seqs):
            ...
            GRU_group = train_and_evaluate_gru(train_sequences, test_sequences, hidden_size=hidden_size, return_pred=True, vanilla_RNN=vanilla_RNN)['sequence_pairs']
...
#%%
### 4 - Group-wise vanilla RNN or GRU Predictions
### with (for train sequences) all sequences
### except the sequences with the same outcome as the test sequences, 
### except the sequences with the same subject than the test sequences' one
...
for hidden_size in [32, 512, 1024]:
    ...
    if vanilla_RNN:
        model = f'big_group_RNN_{hidden_size}'
    else:
    ...
    def big_group_gru_prediction(subject, i):
        ...
        for m in range(nb_test_seqs):
            ...
            GRU_big_group = train_and_evaluate_gru(train_sequences, test_sequences, hidden_size=hidden_size, return_pred=True, vanilla_RNN=vanilla_RNN)['sequence_pairs']
...
p_c_optimal = False # if True, use optimal p_c for HMM without fitting p_c individually to each subject's data 
...
def hmm_subject_prediction(subject, i):
    ...
    for n in range(nb_test_seqs):
        ...
        HMM_subject = predict_sequences_with_HMM(train_sequences, test_sequences, int(1 / (1 - train_size_ratio)), p_c_optimal=p_c_optimal)
...
for model in ['group_HMM', 'big_group_HMM']:
    ...
    def hmm_group_prediction(subject, i):
        ...
        for n in range(nb_test_seqs):
            ...
            HMM_subject = predict_sequences_with_HMM(train_sequences, test_sequences, int(1 / (1 - train_size_ratio)))
            start_idx = n_test_seqs*n_tri*n
            preds[start_idx:start_idx + len(HMM_subject)] = HMM_subject
        return subject, subject_indices, preds
...
#%%
### 7 - Fit NN to HMM to approximate decision function - group level
...
### remain unchanged.
for level in ['big_group', 'subject']:
    
    if level == 'big_group':
        nb_units = [4, 8, 16, 32, 512, 1024]
    elif level == 'subject':
        nb_units = [32, 512, 1024]
    
    for nb_unit in nb_units:
        data = pd.read_csv(data_path)
        ref_model = 'subject_HMM' # better performance than 'group_HMM'
        target = 'estimate'
        model = f'subject_HMM_with_FNN_{nb_unit}_{level}_level'
        ...
        def predict_decision_function(train_data, test_data, nb_unit):
            ...
            # Train and predict
            mlp = MLPRegressor(hidden_layer_sizes=(nb_unit,), activation='logistic', solver='adam', max_iter=max_iter, random_state=random_state)
        ...
        def process_subject_group_augmented_HMM(subject, i):
            ...
            for n in range(nb_test_seqs):
                ...
                preds = predict_decision_function(train_data, test_data, nb_unit)
                print("preds: ", preds)
